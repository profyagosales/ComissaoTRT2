...
export function HeroCard({ left, right }: HeroProps) {
  return (
    <section className="w-full overflow-hidden rounded-3xl border border-red-200 bg-white/95 shadow-lg shadow-red-900/10">
      ...
      <div className="flex flex-col gap-6 px-8 py-6 md:flex-row md:items-start md:justify-between">
        ...
        {right ? (
          <div className="w-full max-w-sm rounded-2xl border border-white/60 bg-red-50/60 px-5 py-4">{right}</div>
        ) : null}
      </div>
    </section>
  )
