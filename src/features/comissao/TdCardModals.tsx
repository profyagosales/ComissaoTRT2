...
import { createManualTdAction, moderateTdRequestAction, upsertTdContentAction } from "./comissao-actions"
...
function ActionButton({ children }: { children: ReactNode }) {
  return (
    <button
      type="button"
      className="w-full rounded-full border border-amber-200 bg-white/90 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-amber-700 shadow-sm transition hover:border-amber-300"
    >
      {children}
    </button>
  )
...
function TdPendenciasModal({ pending }: TdPendenciasModalProps) {
  ...
  const handleDecision = (requestId: string, decision: "APROVAR" | "REJEITAR") => {
    startTransition(async () => {
      ...
      setFeedback(null)
      try {
        await moderateTdRequestAction({ requestId, decision })
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className={cn(dialogContentClasses, "space-y-6")}> 
        <DialogHeader>
          <DialogTitle className="text-2xl font-semibold text-zinc-900">TDs aguardando decisão</DialogTitle>
          <DialogDescription className="text-sm text-zinc-500">
            Aprove ou recuse os envios feitos pelos aprovados. As decisões atualizam o status dos candidatos automaticamente.
          </DialogDescription>
        </DialogHeader>
...
function TdContentEditorModal({ content }: TdContentEditorModalProps) {
  const router = useRouter()
  const [open, setOpen] = useState(false)
  ...
  return (
    <Dialog open={open} onOpenChange={handleDialogChange}>
      <DialogTrigger asChild>
        <ActionButton>Editar conteúdo</ActionButton>
      </DialogTrigger>
      <DialogContent className={cn(dialogContentClasses, "space-y-6")}> 
        <DialogHeader>
          <DialogTitle className="text-2xl font-semibold text-zinc-900">Editor de conteúdo do TD</DialogTitle>
          <DialogDescription className="text-sm text-zinc-500">
            Atualize os textos exibidos para todos os aprovados na página de TDs.
          </DialogDescription>
        </DialogHeader>
...
function TdManualModal({ candidates }: TdManualModalProps) {
  const router = useRouter()
  const [open, setOpen] = useState(false)
  const [form, setForm] = useState({
    candidateId: "",
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <ActionButton>Cadastro manual</ActionButton>
      </DialogTrigger>
      <DialogContent className={cn(dialogContentClasses, "space-y-6")}>
        <DialogHeader>
          <DialogTitle className="text-2xl font-semibold text-zinc-900">Registrar TD manual</DialogTitle>
          <DialogDescription className="text-sm text-zinc-500">
            Use este fluxo quando a comissão receber TDs por e-mail ou outro canal externo.
          </DialogDescription>
        </DialogHeader>
...
export function TdCardActions({ pending, candidates, content }: { pending: PendingTdRequest[]; candidates: CandidateSummary[]; content: TdContentSettings }) {
  return (
    <div className="grid gap-2">
      <TdPendenciasModal pending={pending} />
      <TdContentEditorModal content={content} />
      <TdManualModal candidates={candidates} />
    </div>
  )
}
